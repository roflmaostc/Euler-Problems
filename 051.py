...
def solve_problem():
    ...
    for number in ps:
        for pos in reversed(range(1, len(str(number))+1)):
            tmp = check_if_fulfilled(number, [pos], fam_value, primes)
            if tmp != -1:
                return tmp

            for pos2 in reversed(range(1, pos+1)):
                tmp = check_if_fulfilled(number, [pos, pos2], fam_value, primes)
                if  tmp != -1:
                    return tmp

                for pos3 in reversed(range(1, pos2+1)):
                    tmp = check_if_fulfilled(number, [pos, pos2, pos3], fam_value, primes)
                    if  tmp != -1:
                        return tmp
    return "Nothing found fullfilling criteria"

def check_if_fulfilled(number, l, fam_value, primes):
...
def replace_digit2(n, d, pos):
    """Replaces digit in number n at all position (stored in pos-list) with digit d.
    Approximately 30% faster than replace_digit. 1023.
    Digit is 3 is at pos=1 and digit 1 is at pos=4"""
    s = str(n)
    for p in pos:
        tmp = int(s[len(s)-p])
        n += 10**(p-1)*(d-tmp)
